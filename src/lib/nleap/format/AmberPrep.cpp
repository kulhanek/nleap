...
CAmberPrep::CAmberPrep( CVerboseStr& debug )
    : m_debug( debug )
{
    m_line_no = 0;
}
...
void CAmberPrep::Read( istream& is, CUnitPtr& unit, int& top_id )
{
    if( ! unit ){
        // invalid ff
        throw runtime_error( "unit is NULL in amber_prep_t::read" );
    }
...
//    void read_amber_prep(istream& is, CDatabase& mdb)
//    {
